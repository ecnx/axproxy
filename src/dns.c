 ...
 */
static int nsaddr_internal ( const char *hostname, unsigned int *addr )
{
    size_t attempts = 0;
...
#define CACHE_NAME_LENGTH 48
#define CACHE_RECORDS_LIMIT 128
...
int nsaddr ( const char *hostname, unsigned int *addr )
{
    ...
    if ( strlen ( hostname ) >= sizeof ( name ) )
    {
        return -1;
    }
    ...
    if ( nsaddr_internal ( hostname, addr ) < 0 )
    {
        return -1;
    }
